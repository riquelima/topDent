...
const YesNoDetailsField: React.FC<YesNoDetailsProps> = ({ 
  ...
  if (readOnly) {
    return (
      <div className="mb-3 p-3 border border-gray-700 rounded-md">
...
export const PatientAnamnesisPage: React.FC = () => {
  ...
  const renderReadOnlyView = () => (
    <div className="space-y-6">
        ...
        <div className="p-3 border border-gray-700 rounded-md">
            <p className="text-sm font-medium text-gray-400">DOENÇA:</p>
        ...
        <div>
            ...
            {existingBPReadings.length > 0 ? existingBPReadings.map((bp, idx) => (
                <div key={bp.id || idx} className="grid grid-cols-2 gap-4 mb-2 p-3 border border-gray-700 rounded-md">
                    <div><span className="text-gray-400">Data {idx+1}:</span> {bp.date}</div>
                    <div><span className="text-gray-400">Pressão {idx+1}:</span> {bp.value}</div>
                </div>
            )) : <p className="text-gray-400">Nenhum registro de pressão arterial.</p>}
        </div>
        <Button onClick={() => setIsEditMode(true)} variant="primary">Editar Anamnese</Button>
    </div>
  );
  ...
  const renderForm = () => (
    <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <YesNoDetailsField 
            ...
            onValueChange={(val) => handleFormChange('allergies_exist', val)}
        ...
        <div>
            <Select id="hasDisease" label="DOENÇA:" value={formData.has_disease || ''}
            ...
            {formData.has_disease === 'Sim' && (
              <div className="mt-4 p-4 bg-gray-800 rounded-md space-y-3">
                ...
                <Textarea label="ESPECIFICAR (Outras):" value={formData.disease_other_details}
        ...
        <div>
            ...
            {formBPReadings.map((reading, index) => (
              <div key={index} className="grid grid-cols-1 sm:grid-cols-[1fr_1fr_auto] gap-4 mb-3 p-3 border border-gray-700 rounded-md items-end">
                <DatePicker label={`DATA ${index + 1}`} value={reading.date}
                  onChange={(e) => handleBPChange(index, 'date', e.target.value)} disabled={isLoading}
                />
                <Input label={`PRESSÃO ${index + 1}`} value={reading.value}
                  onChange={(e) => handleBPChange(index, 'value', e.target.value)} disabled={isLoading}
                  placeholder="Ex: 120/80"
                />
                <Button type="button" variant="danger" size="sm" onClick={() => removeBPReadingField(index)} disabled={isLoading || formBPReadings.length <=1 } className="mb-4 h-10">
        ...
        <div className="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-4 pt-6">
            {existingAnamnesis && <Button type="button" variant="ghost" onClick={() => {setIsEditMode(false); populateFormFromExistingData(existingAnamnesis, existingBPReadings);}} disabled={isLoading}>Cancelar Edição</Button>}
            <Button type="button" variant="secondary" onClick={() => navigate(`/patient/${patientId}`)} leftIcon={<ArrowUturnLeftIcon />} disabled={isLoading}>
              Voltar Detalhes Paciente
            </Button>
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <Card title={`Anamnese de ${patient.fullName} (CPF: ${patient.cpf})`}>
        {isEditMode ? renderForm() : renderReadOnlyView()}
      </Card>
    </div>
  );
};
