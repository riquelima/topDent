...
export const DentistChatWidget: React.FC<DentistChatWidgetProps> = ({ dentistId }) => {
  ...
  const playNotificationSound = useCallback(() => {
    ...
    if (audio) {
        ...
        audio.play().catch(error => {
            ...
            // Fallback using Web Audio API if direct play fails
            try {
                ...
                console.error("Audio fallback failed for dentist", fallbackError);
            }
        });
    }
  }, []);
  ...
  const toggleChat = () => {
    if (!audioUnlockedRef.current && audioRef.current) {
        ...
        audio.play().then(() => {
            ...
            console.log("🔊 Áudio desbloqueado para notificações futuras (dentista).");
        }).catch((err) => {
  ...
  return (
    <>
      {isOpen && (
        <div ref={widgetRef} className="fixed bottom-24 right-8 w-full max-w-lg h-[700px] bg-[#1a1a1a] rounded-xl shadow-2xl flex flex-col z-50 border border-gray-700/50 animate-slide-up-widget">
